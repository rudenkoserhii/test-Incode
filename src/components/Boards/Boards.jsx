...
import {
  getToDoIssues,
  nextPageToDoIssues,
} from "../../redux/toDoIssues/slice";
import {
  getInProgressIssues,
  nextPageInProgressIssues,
} from "../../redux/inProgressIssues/slice";
import {
  getDoneIssues,
  nextPageDoneIssues,
} from "../../redux/doneIssues/slice";
        ...
        try {
          ...
          if (changes.some((a) => a.repo === repo)) {
            ...
            changes
              .find((e) => e.repo === repo)
              .data.forEach(
                (f) => f.columnIn === "ToDo" && filtered.push(f.issue)
              );
          } else {
            filtered = toDo.data;
          }
      ...
      (async function getData() {
        ...
        try {
          ...
          if (!inProgress.data) {
            ...
            return Notiflix.Notify.failure(
              "Whoops, no In Progress issues more!"
            );
          }
            ...
            filtered = inProgress.data.filter(
              (a) =>
                !changes
                  .find((b) => b.repo === repo)
                  .data.some(
                    (d) => a.id === d.id && "In Progress" === d.columnOut
                  )
            );
            changes
              .find((e) => e.repo === repo)
              .data.forEach(
                (f) => f.columnIn === "In Progress" && filtered.push(f.issue)
              );
          } else {
        ...
        try {
          ...
          if (changes.some((a) => a.repo === repo)) {
            ...
            changes
              .find((e) => e.repo === repo)
              .data.forEach(
                (f) => f.columnIn === "Done" && filtered.push(f.issue)
              );
          } else {
            filtered = done.data;
          }
